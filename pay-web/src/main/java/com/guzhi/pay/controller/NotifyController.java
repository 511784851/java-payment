...
public class NotifyController {
    ...
    @ResponseBody
    public String gbNotify(HttpServletRequest request, HttpServletResponse response) {
        int result;
        try {
            Map<String, String> params = HttpUtils.getParameterMap(request);
            logger.info("gb notify map={}", HttpUtils.map2String(params));
            // 根据回调参数组装payorder
            PayOrder payOrder = gbHelper.assemblePayOrder(resource, params);
            logger.info("[gbNotify] assembled  payOrder={}", payOrder);
            handleNotifyOrder(payOrder);
            result = gbBalanceConsts.SUCCESS;
        } catch (Throwable t) {
            logger.warn("[gbNotify]exception when handling ReturnUrl!", t);
            result = gbBalanceConsts.UNKNOWN_ERROR;
        }
        return result + "";
    }
    ...
    private void createTask(PayOrder payOrder) {
        ...
        if ((!StringUtils.isEmpty(payOrder.getgbOper()))
            ...
            } else if (Consts.gbOper.ADD_DEPOSIT.equals(payOrder.getgbOper())) {
                // add deposit
                taskType = Task.TYPE_ADD_DEPOSIT;
                task = resource.getTask(payOrder.getAppId(), payOrder.getAppOrderId(), taskType);
            } else if (Consts.gbOper.ADD_CHANNEL_DEPOSIT.equals(payOrder.getgbOper())) {
                // add channel deposit
