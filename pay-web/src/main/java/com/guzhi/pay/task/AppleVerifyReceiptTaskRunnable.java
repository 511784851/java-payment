...
public class AppleVerifyReceiptTaskRunnable implements Runnable {
    ...
    private static final String gbUID = "774263341";
    private Task task;
    ...
    public void run() {
        try {
            ...
            // 测试环境只有当gbuid 为774263341 才进行测试
            if (gbUID.equalsIgnoreCase(JsonHelper.fromJson(payOrder.getUserId(), Consts.gbUID))) {
    ...
    private void handlerSuccess(PayOrder payOrder) {
        ...
        if (Consts.gbOper.ADD.equalsIgnoreCase(payOrder.getgbOper())) {
            resource.createTask(new Task(payOrder.getAppId(), payOrder.getAppOrderId(), Task.TYPE_ADD_GB, payOrder
                    .getChId(), payOrder.getPayMethod()));
        } else {
            resource.createTask(new Task(payOrder.getAppId(), payOrder.getAppOrderId(), Task.TYPE_NOTIFY, payOrder
                    .getChId(), payOrder.getPayMethod()));
