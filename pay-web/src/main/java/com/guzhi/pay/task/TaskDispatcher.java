...
public class TaskDispatcher {
    ...
    public void doService(Task task) {
        ...
        } else if (Task.TYPE_ADD_GB.equals(task.getType())) { // 充值gb币
            ttp.execute(new AddgbTaskRunable(task, resource, adapterSelector));
        } else if (Task.TYPE_ADD_DEPOSIT.equals(task.getType())) {// 增加保证金
            ttp.execute(new AddDepositTaskRunable(task, resource, adapterSelector));
        } else if (Task.TYPE_ADD_CHANNEL_DEPOSIT.equals(task.getType())) {// 增加频道保证金
            ttp.execute(new AddChannelDepositTaskRunnable(task, resource, adapterSelector));
