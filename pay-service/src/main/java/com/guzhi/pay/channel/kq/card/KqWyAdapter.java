 ...
 */
@Service("kqWyAdapter")
public class KqWyAdapter extends KqCardAdapter implements ChannelIF {

    private static final Logger LOG = LoggerFactory.getLogger(KqWyAdapter.class);
    ...
    public String getBossType() {
        return KqConsts.KQ_WY_BOSS_TYPE;
    }

    @Override
    public String getPayType() {
        return KqConsts.KQ_WY_CARD_PAY_TYPE;
    }

    @Override
    public void updatePayOrder(String payUrl, PayOrder order) {
        LOG.info("[KqWyAdapter.updatePayOrder] with payUrl: {}", payUrl, TraceHelper.getTrace(order));
        order.setPayUrl(payUrl);
        order.setStatusCode(Consts.SC.PENDING);
        order.setStatusMsg("等待用户支付订单，或等待快钱通知");
        LOG.info("[KqWyAdapter.updatePayOrder] return PayOrder: {}", order, TraceHelper.getTrace(order));
    }
