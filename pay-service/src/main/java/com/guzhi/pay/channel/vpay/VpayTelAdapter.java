...
public class VpayTelAdapter extends AbstractChannelIF {
    ...
    public PayOrder pay(PayOrder order) {
        ...
        String gbuid = JsonHelper.fromJson(order.getUserId(), VpayConsts.KEY_gbUID);
        ...
        request.put(VpayConsts.KEY_USER_ID, gbuid);
        ...
        request.put(VpayConsts.KEY_NOTIFY_URL, UrlHelper.removeLastSep(getguzhiPayNotify()) + VpayConsts.VB_NOTIFY);
        // gen sign
        String signMsg = VpayHelper.genSignForTel(request, appChInfo.getChPayKeyMd5());
        request.put(VpayConsts.KEY_SP_MD5, signMsg);
        // gen payUrl
        String payUrl = UrlHelper.removeLastSep(getguzhiPayNotify() + VpayConsts.VPAYTEL_FRONT) + "?"
