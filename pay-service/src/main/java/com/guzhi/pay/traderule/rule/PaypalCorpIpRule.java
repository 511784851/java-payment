...
public class PaypalCorpIpRule implements RuleValidator {
    ...
    public Map<String, String> validator(Map<String, String> params) {
        ...
        if (result >= prohibitLimit) {
            LOG.info("[PaypalCorpIpRule.validator] Paypal:{},gbuid:{} corresponding {} ip, ip:{},refuse request",
                    account, gbuid, result, ip, TraceHelper.getTrace(params));
            returnFlag = true;
        }

        if (result >= blackLimit) {
            LOG.info("[PaypalCorpIpRule.validator] Paypal:{},gbuid:{} corresponding {} ip, ip:{},add to black",
                    account, gbuid, ip, TraceHelper.getTrace(params));
            returnFlag = true;
