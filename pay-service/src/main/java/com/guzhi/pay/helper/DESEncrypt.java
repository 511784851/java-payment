...
public class DESEncrypt {
    ...
    public static String encryptBy3DES(String src, String key) {
        ...
        byte[] kegb = key.getBytes();
        byte[] encodedB = encryptMode(kegb, srcB);
    ...
    public static String decryptBy3DES(String encryptText, String secretKey) {
        ...
        try {
            ...
            byte[] decodedBytes = Base64.decodeBase64(encryptText);
     ...
     * kegbyte为加密密钥，长度为24字节
     * src为被加密的数据缓冲区（源）
     */
    public static byte[] encryptMode(byte[] kegbyte, byte[] src) {
        try {
            SecretKey deskey = new SecretKeySpec(kegbyte, DESEDE);
    ...
    private static String encode(String str, String type) {
        try {
            ...
            throw new PayException(Consts.SC.INTERNAL_ERROR, "str = " + str + "  ," + "type = " + type + "  ,"
                    + e.getMessage());
        }
    }
