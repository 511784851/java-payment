...
public interface UserTransInfoMapper {
    @Select("select sum(amount) from user_trans_info where yyuid=#{yyuid} and chId=#{chId} and status='success'")
    String getHisTotalAmount(@Param("yyuid") String yyuid, @Param("chId") String chId);

    @Select("select sum(amount) from user_trans_info where yyuid=#{yyuid} and chId=#{chId} and status='success' and paytime > #{startTime} and paytime < #{endTime}")
    String getHisTotalAmountByTime(@Param("yyuid") String yyuid, @Param("chId") String chId,
    ...
    @Select("SELECT COUNT(*) FROM (SELECT DISTINCT(account) FROM user_trans_info WHERE yyuid=#{yyuid}  and account!=#{exAccount} and chId=#{chId} and status='success' and paytime > #{startTime} and paytime < #{endTime}) t")
    int getYyCrospPaypal(@Param("yyuid") String yyuid, @Param("chId") String chId,
    ...
    @Select("select count(*) from user_trans_info where yyuid=#{yyuid} and chId=#{chId} and status='success' and  paytime < #{endTime}")
    int getPayTimesByEndTime(@Param("yyuid") String yyuid, @Param("chId") String chId, @Param("endTime") String endTime);

    @Insert({
            "insert into user_trans_info(`appId`,`appOrderId`,`yyuid`,`account`,`paytime`,`ip`,`address`,",
            "`ext`,`amount`,`chId`,`status`,`statusMsg`,`lastUpdateTime`) values(",
            "#{appId},#{appOrderId},#{yyuid},#{account},#{payTime},#{ip},#{address},#{ext},#{amount},#{chId},#{status},#{statusMsg},#{lastUpdateTime})" })
