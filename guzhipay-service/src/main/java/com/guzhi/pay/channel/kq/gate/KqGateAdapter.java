...
public class KqGateAdapter extends AbstractKqAdatper implements ChannelIF {
    ...
    public PayOrder pay(PayOrder order) {
        ...
        request.put(KqConsts.KEY_BGURL, UrlHelper.removeLastSep(getPayNotify()) + KqConsts.ADDR_YYPAY_NOTIFY); // UrlHelper.removeLastSep("http://172.19.51.71")
        request.put(KqConsts.KEY_PAGEURL, UrlHelper.removeLastSep(getPayNotify()) + KqConsts.ADDR_YYPAY_RETURN);
        ...
        // 构建pay url
        String payUrl = UrlHelper.removeLastSep(getPayKqAddr() + KqConsts.ADDR_PAY) + "?"
    ...
    public String getQueryAddress() {
        return getPayKqAddr() + KqConsts.ADDR_QUERY;
    }
}
