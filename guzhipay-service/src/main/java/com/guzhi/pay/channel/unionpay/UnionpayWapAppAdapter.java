...
public class UnionpayWapAppAdapter extends AbstractChannelIF {
    ...
    public PayOrder pay(PayOrder payOrder) {
        ...
        req.put(Constants.KEY_BACKENDURL, getPayNotify() + Constants.APP_NOTIFYURL);
        // 银联暂时不提供前台通知的功能
        req.put(Constants.KEY_FRONTENDURL, getPayNotify() + Constants.APP_RETURNURL);
        req.put(Constants.KEY_ORDERDESCRIPTION, payOrder.getProdName());// 订单描述(可选)
        req.put(Constants.KEY_ORDERTIME, sdf.format(new Date()));// 交易开始日期时间yyyyMMddHHmmss
