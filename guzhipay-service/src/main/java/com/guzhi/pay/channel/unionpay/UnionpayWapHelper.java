...
public class UnionpayWapHelper {
    ...
    public static String generatePayRequestBody(PayOrder payOrder, Map<String, String> request) {
        ...
        try {
            String base64DesKey = Base64.encode(EncryptUtil.encryptByRSA(desKey.getBytes(Consts.CHARSET_UTF8),
                    UnionpayWapAdapter.PRIVATEKEYFILEPATH, payKey));
            String base64Content = Base64
                    .encode(EncryptUtil.encryptBy3DES(desKey.getBytes(), content.getBytes(Consts.CHARSET_UTF8)));
    ...
    private static String baseMerchantId(PayOrder payOrder) {
        ...
        try {
            base64MerchantId = new String(
                    Base64.encode(payOrder.getAppChInfo().getChAccountId().getBytes(Consts.CHARSET_UTF8)));
    ...
    public static Map<String, String> generateNotifyRequestMap(HttpServletRequest req, HttpServletResponse resp) {
        ...
        try {
            ...
            String merchantId = new String(Base64.decode(base64MerchantId));
            byte[] desKey = EncryptUtil.decryptByRSA(Base64.decode(base64DesKey), UnionpayWapAdapter.PUBLICKEYFILEPATH);
            String content = new String(EncryptUtil.decryptBy3DES(desKey, Base64.decode(base64Content)));
            LOG.info(
    ...
    public static String getNotifyResponse(Map<String, String> requestMap) {
        try {
            ...
            String base64RespContent = Base64.encode(
                    EncryptUtil.encryptBy3DES(requestMap.get(Constants.DESKEY).getBytes(), respContent.getBytes()));
            String base64MD5RespContent = Base64.encode(MD5Utils.getMD5(respContent).getBytes());
    ...
    public static void updatePayOrderWithPayResponse(String responseBody, PayOrder payOrder) {
        ...
        if (Constants.ERROR.equalsIgnoreCase(strArray[0])) {
            ...
            String resultMsg = new String(Base64.decode(strArray[2]));
        ...
        try {
            content = new String(
                    EncryptUtil.decryptBy3DES(desKey.getBytes(Consts.CHARSET_UTF8), Base64.decode(strArray[1])));
    ...
    public static String generateQueryRequestBody(PayOrder payOrder) {
        ...
        try {
            base64EncryptDesKey = Base64.encode(EncryptUtil.encryptByRSA(desKey.getBytes(Consts.CHARSET_UTF8),
                    UnionpayWapAdapter.PRIVATEKEYFILEPATH, payKey));
            base64EncryptContent = Base64.encode(EncryptUtil.encryptBy3DES(desKey.getBytes(Consts.CHARSET_UTF8),
    ...
    public static void updatePayOrderWithQueryResponse(String responseBody, PayOrder payOrder) {
        String[] strArray = responseBody.split("\\|");
        if (Constants.ERROR.equalsIgnoreCase(strArray[0])) {
            LOG.error("[updatePayOrderWithQueryResponse] request upbp error,code:{},message:{},payOder:{}", strArray[1],
                    Base64.decode(strArray[2]), payOrder);
            throw new PayException(Consts.SC.DATA_ERROR, "request upbp error", payOrder);
        }
        String desKey = JsonHelper.fromJson(payOrder.getAppChInfo().getAdditionalInfo(), Constants.DESKEY);
        desKey = MD5Utils.getMD5(desKey);
        String content = new String(EncryptUtil.decryptBy3DES(desKey.getBytes(), Base64.decode(strArray[1])));
        // Verifying MD5 sign of content necessary.
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(responseCode) && responseCode.equals("0000")) {
                ...
                // LOG.info("[updatePayOrderWithQueryResponse] pay
                // failed,payOrder:{}",
                // payOrder);
                //
                // }
                    ...
                    LOG.warn("[updatePayOrderWithQueryResponse] pay order not exists in unionpay,payOrder:{}",
                            payOrder);
                }
            } else {
     ...
     * @param payNotify
     */
    public static String generateFrontUrl(PayOrder payOrder, String payNotifyUrl) {
        ...
        String frontUrl = UrlHelper.removeLastSep(payNotifyUrl) + Constants.RETURNURL;
