...
public class PayAmountLimitRule implements RuleValidator {
    ...
    public Map<String, String> validator(Map<String, String> params) {
        ...
        if (StringUtils.isBlank(yyuid) || StringUtils.isBlank(chId) || StringUtils.isBlank(payAmount)) {
            return resultMap;
        }
        String endtime = TimeHelper.nDayDate(-7, TimeHelper.TIME_FORMATE);
        int payTimes = domainResource.getPayTimesByEndTime(yyuid, chId, endtime);
        // 一个星期之前没有充值过,最高金额为30美金
        if (payTimes == 0) {
            if (Double.parseDouble(payAmount) > 30) {
                LOG.info("[PayAmountLimitRule.validator] pay yyuid:{},ammout is {},limit amount is {}", yyuid,
            ...
            // 对于以及充值过的用户,依照旧的规则
            String totalAmount = domainResource.getHisTotalAmount(yyuid, chId);
            ...
            String recentDaytotalAmount = domainResource.getHisTotalAmountByTime(yyuid, chId, startTime, endTime);
