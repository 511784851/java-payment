...
public class TimeInnerOrderRule implements RuleValidator {
    ...
    public Map<String, String> validator(Map<String, String> params) {
        ...
        if (twoHourInner >= 3) {
            LOG.info(
                    "[TimeInnerOrderRule.validator] pay too much in 2 hour,account:{}, yyuid:{},pay time:{} ,times: {}",
                    account, yyuid, endTime, twoHourInner, TraceHelper.getTrace(params));
        ...
        if (hourInner >= 5) {
            LOG.info(
                    "[TimeInnerOrderRule.validator] pay too much in 24 hour,account:{},yyuid:{},pay time:{} ,times: {}",
                    account, yyuid, endTime, hourInner, TraceHelper.getTrace(params));
            resultMap.put(TradeRuleConsts.FLAG, TradeRuleConsts.TRUE);
        ...
        if (Double.parseDouble(recentDaytotalAmount) + Double.parseDouble(payAmount) > 400) {
            LOG.info(
                    "[TimeInnerOrderRule.validator] pay limit ,account:{},yyuid:{},24 hours totalAmount:{},payAmount:{}",
                    account, yyuid, recentDaytotalAmount, payAmount, TraceHelper.getTrace(params));
            resultMap.put(TradeRuleConsts.FLAG, TradeRuleConsts.TRUE);
