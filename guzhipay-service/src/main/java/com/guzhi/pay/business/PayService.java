...
public class PayService {
    ...
    private PayOrder afterPayCompletion(PayOrder payOrder) {
        ...
        if (Consts.SC.SUCCESS.equalsIgnoreCase(payOrder.getStatusCode())) {
            ...
            // 添加日志观察同步下单即订单成功的情况.
            logger.info("[pay] get a synchronous channel,set pending status if yb wapbalance,payOrder:{}", payOrder);
            // 保险起见，这里指针对做手机Y币的订单状态做调整
            if (Consts.Channel.YB.equalsIgnoreCase(payOrder.getChId())
    ...
    public PayOrder query(PayOrder payOrder) {
        ...
        // 如果除了充钱没有其他充值操作则订单成功
        if (Consts.SC.SUCCESS.equalsIgnoreCase(statusCode) && StringUtils.isBlank(payOrderInDb.getYyOper())) {
            return payOrderInDb;
        }
        // invoke channel
        try {
        ...
        public void run() {
            ...
            try {
                ...
                if (adapter != null) {
                    ...
                    // throw new PayException(Consts.SC.INTERNAL_ERROR, "can not
                    // find adpater!", payOrder);
