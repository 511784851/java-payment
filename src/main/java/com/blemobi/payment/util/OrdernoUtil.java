...
import com.blemobi.library.util.Base64;
...
public class OrdernoUtil {
	private final static String key = "hIrH/Ry3r4JMQHRQMl74BzMsvH+sUzkKuSFEUcClErdu2vvoPOf5kgQBa3YHzgXMboVV6seC5aFdr7SRGzJFzQ==";
	...
	public static String build(String uuid, long signTime, long amount) {
		...
		if (uuid == null)
			throw new RuntimeException("UUID  Format Exception! ");
		uuid = uuid.replaceAll("-", "");
		if (uuid.length() < (subUuidStartPoint + subUuidLen))
			throw new RuntimeException("UUID  Format Exception! ");
	...
	private static byte[] converHexTobyte(String data) {
		if (data == null)
			throw new RuntimeException("Hex Source is null");
		int len = data.length();
		if ((len % 2) != 0)
			throw new RuntimeException("Hex Source len must be Multiple of 2");
		int byteLen = len / 2;
	...
	private static byte[] getLastEightByte(byte[] encryptData) {
		if (encryptData == null)
			throw new RuntimeException("LastEightByte Source Data Format Exception! ");
		byte[] rtn = new byte[8];
