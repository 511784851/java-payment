...
public class LotteryDaoImpl extends JdbcTemplate implements LotteryDao {
    ...
    public int paySucc(String ordNo, int amt) {
        String sql = "UPDATE t_lotteries SET status = 2, refund_status = 0 WHERE id = ? AND status = 1 AND tot_amt = ?";
    ...
    public Map<String, Object> queryLotteryInf(String lotteryId) {
        String sql = "SELECT crt_tm, tot_amt, remain_cnt, remain_amt, uuid, status FROM t_lotteries WHERE id = ? ";
    ...
    public List<Map<String, Object>> getExpireLottery(long expTm) {
        String sql = "SELECT id, tot_amt, remain_amt, status, remain_cnt, winners, uuid, crt_tm  FROM t_lotteries WHERE status IN(0, 2) AND crt_tm > ? AND refund_status = 0 AND remain_amt > 0";
