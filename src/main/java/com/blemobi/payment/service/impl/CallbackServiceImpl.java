...
public class CallbackServiceImpl implements CallbackService {
    ...
    public Boolean paySucc(String amount, long time, String ordNo, String recUid, String corgOrdId, String corgSts,
        ...
        Reward reward = null;
        RedSend rs = null;
        if (bizType == OrderEnum.RED_ORDINARY.getValue() || bizType == OrderEnum.RED_GROUP_EQUAL.getValue()
                || bizType == OrderEnum.RED_GROUP_RANDOM.getValue()) {
            // 红包
            log.info("red");
        ...
        ret = transactionDao.insert(new Object[] {uuid, ordNo, bizType + "", money, 1, " ", desc, corgOrdId, corgSts,
                corgMsg, currTm, currTm, ordNo });
        log.info("完成交易流水插入");
        if (ret != 1) {
            throw new RuntimeException("操作数据库异常");
        }
